  ...
  .factory('applicationDataFactory', [function applicationDataFactory() {
    ...
    function filter(category, type) {
      ...
      for (var i = 0, len = recipeNameArray.length; i < len; i++) {
        ...
        console.log("Retrieving recipe: " + recipeNameArray[i]);
        var request = objectStore.get(recipeNameArray[i]);
        ...
        request.onsuccess = function(event) {
          ...
          recipesArray.push(event.target.result);
        };
      }
    ...
    return {
      ...
      setAllRecipes: function(newAllRecipes) {
        ...
        console.log("setAllRecipes called");

        var openDbRequest = window.indexedDB.open("RecipeCollection", 10);
        ...
        openDbRequest.onupgradeneeded = function(event) {
          ...
          var oldObjectStore = db.deleteObjectStore("recipes");
          var objectStore = db.createObjectStore("recipes", {
            keyPath: "filename"
          });
        }
        ...
        openDbRequest.onsuccess = function(event) {
          ...
          for (var key in newAllRecipes) {
            var recipe = newAllRecipes[key];
            var addRequest = objectStore.put(recipe);
          }
        };
